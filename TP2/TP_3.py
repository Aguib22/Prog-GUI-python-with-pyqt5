    ...
    def mouseReleaseEvent(self, event):
        ...
        if self.doPaint:
            ...
            if 0 <= index <= (ord('Z') - ord('A')):
                ...
                print(f"Letter clicquer: {letter}")
                self.sendLettre.emit(letter)
...
class myBrowserWindow(QWidget):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("SOW " + datetime.now().strftime("  %H:%M:%S"))
        self.resize(800, 600)

        # widget navigateur occupant tout l'espace
        ...
        # URL de base
        self.base_url = 'https://bulbapedia.bulbagarden.net/wiki/List_of_Pokémon_by_name'
        # charger la page initiale
        self.myBrowserWidget.load(QUrl(self.base_url))

    def changeLetter(self, letter: str):
        # concatène l'ancre et recharge
        newurl = self.base_url + '#' + letter
        print("chargement:", newurl)
        self.myBrowserWidget.load(QUrl(newurl))
...
if __name__ == '__main__':
    ...
    w = myMainWindow() 
    b = myBrowserWindow()
    w.sendLettre.connect(b.changeLetter)
    b.show()
    w.show() 
    sys.exit(app.exec_())
# ...existing code...